...
fn test_boxed_slices() {
  ...
  assert_eq!(
    try_cast_slice_box::<u8, ()>(Box::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_box::<u8, ()>(Box::new([1, 2, 3])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_box::<(), i8>(Box::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_box::<(), i8>(Box::new([(); 42])).unwrap_err().0,
    PodCastError::SizeMismatch
  );
}
...
fn test_rc_slices() {
  ...
  assert_eq!(
    try_cast_slice_rc::<u8, ()>(Rc::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_rc::<u8, ()>(Rc::new([1, 2, 3])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_rc::<(), i8>(Rc::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_rc::<(), i8>(Rc::new([(); 42])).unwrap_err().0,
    PodCastError::SizeMismatch
  );
}
...
fn test_arc_slices() {
  ...
  assert_eq!(
    try_cast_slice_arc::<u8, ()>(Arc::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_arc::<u8, ()>(Arc::new([1, 2, 3])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_arc::<(), i8>(Arc::new([])).unwrap_err().0,
    PodCastError::SizeMismatch
  );

  assert_eq!(
    try_cast_slice_arc::<(), i8>(Arc::new([(); 42])).unwrap_err().0,
    PodCastError::SizeMismatch
  );
}
